...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "CHNCoin server has now stopped running!";
}
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid CHNCoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid CHNCoin address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; CHNCoin server stopping, restart to run with encrypted wallet";
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "CHNCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "CHNCoin is downloading blocks...");
